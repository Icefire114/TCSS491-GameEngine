...
export class GiantZombie extends Zombie {
    ...
    constructor(pos?: Vec2) {
        const tag: string = "GiantZombie";
        const attack_range: number = 4;
        const attack_cooldown: number = 2.0;
        const run_range: number = 7;
        const health: number = 300;
    ...
    protected _onUpdate(keys: { [key: string]: boolean; }, deltaTime: number, player: Player, mountain: Mountain, distance: number, currentTime: number): void {
        // No specific update logic for GiantZombie beyond what's in the base class.
    }
